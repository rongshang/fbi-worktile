...
function dhtmlXTreeObject(a, b, c, d) {
    ...
    if (_isMacOS) this.allTree.oncontextmenu = function(a) {
...
dhtmlXTreeObject.prototype._doContClick = function(a, b) {
    if (!b && a.button != 2) return this._acMenu && (this._acMenu.hideContextMenu ? this._acMenu.hideContextMenu() : this.cMenu._contextEnd()),
        !0;
    ...
    if (this._acMenu = d.cMenu || this.cMenu) {
        if (!this.callEvent("onBeforeContextMenu", [d.id])) return ! 0; (a.srcElement || a.target).oncontextmenu = function(a) { (a || event).cancelBubble = !0;
            return ! 1
        };
...
function dhtmlXTreeItemObject(a, b, c, d, f, e) {
    ...
    this.htmlNode = this.treeNod.checkBoxOff ? this.treeNod._createItem(1, this, e) : this.treeNod._createItem(0, this, e);
    this.htmlNode.objBelong = this;
    return this
}
dhtmlXTreeObject.prototype._globalIdStorageAdd = function(a, b) {
    if (this._globalIdStorageFind(a, 1, 1)) return a = a + "_" + (new Date).valueOf(),
...
dhtmlXTreeObject.prototype._attachChildNode = function(a, b, c, d, f, e, g, h, j, i, m) {
    ...
    var n;
    if (! (n = this._idpull[b]) || n.span != -1) n = k[l] = new dhtmlXTreeItemObject(b, c, a, this, d, 1),
        b = k[l].id,
        a.childsCount++;
    if (!n.htmlNode) n.label = c,
    ...
    i && i.tr && i.tr.nextSibling ? a.htmlNode.childNodes[0].insertBefore(q, i.tr.nextSibling) : this.parsingOn == a.id ? this.parsedArray[this.parsedArray.length] = q: a.htmlNode.childNodes[0].appendChild(q);
    i && !i.span && (i = null);
    if (this.XMLsource) n.XMLload = j && j != 0 ? 0 : 1;
    n.tr = q;
    q.nodem = n;
    if (a.itemId == 0) q.childNodes[0].className = "hiddenRow"; (a._r_logic || this._frbtr) && this._setSrc(n.htmlNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0], this.imPath + this.radioArray[0]);
    if (h) {
        s = h.split(",");
    ...
    if (!this.XMLloadingWarning && (this._getOpenState(a) < 0 && !this._hAdI && this.openItem(a.id), i && (this._correctPlus(i), this._correctLine(i)), this._correctPlus(a), this._correctLine(a), this._correctPlus(n), a.childsCount >= 2 && (this._correctPlus(k[a.childsCount - 2]), this._correctLine(k[a.childsCount - 2])), a.childsCount != 2 && this._correctPlus(k[0]), this.tscheck && this._correctCheckStates(a), this._onradh)) if (this.xmlstate == 1) {
        var t = this.onXLE;
...
dhtmlXTreeObject.prototype._parseItem = function(a, b, c, d) {
    ...
    if (this._srnd && (!this._idpull[f = a.get("id")] || !this._idpull[f].span)) this._addItemSRND(b.id, f, a);
        ...
        if (typeof this.waitUpdateXML == "object" && !this.waitUpdateXML[e.id]) this._parse(a, e.id, 1);
            ...
            e.open && g.push("OPEN");
            if (this.waitUpdateXML) if (this._globalIdStorageFind(e.id)) var h = this.updateItem(e.id, e.text, e.im0, e.im1, e.im2, e.checked, e.child);
            else this.npl == 0 ? g.push("TOP") : c = b.childNodes[this.npl],
                    h = this._attachChildNode(b, e.id, e.text, 0, e.im0, e.im1, e.im2, g.join(","), e.child, 0, c),
                    e.id = h.id,
                    c = null;
            else h = this._attachChildNode(b, e.id, e.text, 0, e.im0, e.im1, e.im2, g.join(","), e.child, d || 0, c);
            ...
            e.locked == "1" && this.lockItem(h.id, !0, !0); (e.imwidth || e.imheight) && this.setIconSize(e.imwidth, e.imheight, h); (e.closeable == "0" || e.closeable == "1") && this.setItemCloseable(h, e.closeable);
            var i = "";
...
dhtmlXTreeObject.prototype._parse = function(a, b, c, d) {
    ...
    if (a.exists()) {
        ...
        if (!c) {
            ...
            for (var i = this._globalIdStorageFind(this.parsingOn), j = 0; j < this.parsedArray.length; j++) g.htmlNode.childNodes[0].appendChild(this.parsedArray[j]);
            this.parsedArray = [];
...
dhtmlXTreeObject.prototype._branchUpdateNext = function(a) {
    a.each("item",
        function(a) {
            var c = a.get("id");
            if (!this._idpull[c] || this._idpull[c].XMLload) this._branchUpdate++,
...
dhtmlXTreeObject.prototype._correctLine = function(a) {
    if (a.htmlNode) {
        ...
        if (b) if (this._getLineStatus(a.id, b) == 0 || !this.treeLinesOn) for (var c = 1; c <= a.childsCount; c++) {
            ...
            if (!a.htmlNode.childNodes[0].childNodes[c]) break;
            a.htmlNode.childNodes[0].childNodes[c].childNodes[0].style.backgroundImage = "url(" + this.imPath + this.lineArray[5] + ")";
...
dhtmlXTreeObject.prototype._HideShow = function(a, b) {
    if (this.XMLsource && !a.XMLload) {
        ...
        var c = a.htmlNode.childNodes[0].childNodes,
            d = c.length;
        if (d > 1) { (c[1].style.display != "none" || b == 1) && b != 2 ? (this.allTree.childNodes[0].border = "1", this.allTree.childNodes[0].border = "0", nodestyle = "none") : nodestyle = "";
...
dhtmlXTreeObject.prototype._getIndex = function(a) {
    for (var b = a.parentObject,
             c = 0; c < b.childsCount; c++) if (b.childNodes[c] == a) return c
};
...
dhtmlXTreeObject.prototype.onRowSelect = function(a, b, c) {
    ...
    var f = d.treeNod,
        e = f.getSelectedItemId(); (!a || !a.skipUnSel) && f._selectItem(d, a);
...
dhtmlXTreeObject.prototype._correctCheckStates = function(a) {
    if (this.tscheck && a && a.id != this.rootId) {
        var b = a.childNodes,
            ...
            d = 0;
        if (a.childsCount != 0) {
            for (var f = 0; f < a.childsCount; f++) if (!b[f].dscheck) if (b[f].checkstate == 0) c = 1;
...
dhtmlXTreeObject.prototype._createItem = function(a, b, c) {
    ...
    if (this.dragAndDropOff) {
        if (this._aimgs) this.dragger.addDraggableItem(m, this),
            m.parentObject = b;
    ...
    f.appendChild(e);
    d.appendChild(f);
    if (this.ehlt || this.checkEvent("onMouseIn") || this.checkEvent("onMouseOut")) e.onmousemove = this._itemMouseIn,
        e[_isIE ? "onmouseleave": "onmouseout"] = this._itemMouseOut;
    return d
};
...
dhtmlXTreeObject.prototype._recreateBranch = function(a, b, c) {
    ...
    if (e) this._onradh = e,
        this._onradh(g.id);
    for (d = 0; d < a.childsCount; d++) this._recreateBranch(a.childNodes[d], g, 0, 1);
    return g
};
...
dhtmlXTreeObject.prototype._openItem = function(a) {
    ...
    if (b < 0 || this.XMLsource && !a.XMLload) {
        ...
        this._HideShow(a, 2);
        if (this.checkEvent("onOpenEnd")) this.onXLE == this._epnFHe && this._epnFHe(this, a.id, !0),
...
dhtmlXTreeObject.prototype._getLeafCount = function(a) {
    for (var b = 0,
             c = 0; c < a.childsCount; c++) a.childNodes[c].childsCount == 0 && b++;
    return b
};
...
dhtmlXTreeObject.prototype._getAllScraggyItems = function(a) {
    for (var b = "",
             c = 0; c < a.childsCount; c++) if (a.childNodes[c].unParsed || a.childNodes[c].childsCount > 0) {
...
dhtmlXTreeObject.prototype._setCheck = function(a, b) {
    if (a) {
        if ((a.parentObject._r_logic || this._frbtr) && b) if (this._frbtrs) this._frbtrL && this.setCheck(this._frbtrL.id, 0),
            this._frbtrL = a;
        else for (var c = 0; c < a.parentObject.childsCount; c++) this._setCheck(a.parentObject.childNodes[c], 0);
...
dhtmlXTreeObject.prototype._setSubChecked = function(a, b) {
    ...
    if (b) {
        ...
        else for (c = 0; c < b.childsCount; c++) this._setSubChecked(a, b.childNodes[c]);
...
dhtmlXTreeObject.prototype._globalIdStorageRecSub = function(a) {
    for (var b = 0; b < a.childsCount; b++) this._globalIdStorageRecSub(a.childNodes[b]),
...
dhtmlXTreeObject.prototype.enableDragAndDrop = function(a, b) {
    a == "temporary_disabled" ? (this.dADTempOff = !1, a = !0) : this.dADTempOff = !0; (this.dragAndDropOff = convertStringToBoolean(a)) && this.dragger.addDragLanding(this.allTree, this);
...
dhtmlXTreeObject.prototype._createDragNode = function(a, b) {
    ...
    var d = document.createElement("div"),
        f = [];
    if (this._itim_dg) for (var e = 0; e < this._selected.length; e++) f[e] = "<table cellspacing='0' cellpadding='0'><tr><td><img width='18px' height='18px' src='" + this._getSrc(this._selected[e].span.parentNode.previousSibling.childNodes[0]) + "'></td><td>" + this._selected[e].span.innerHTML + "</td></tr></table>";
...
dhtmlXTreeObject.prototype._drag = function(a, b, c) {
    ...
    if (!c.parentObject) c = this.htmlNode.htmlNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0],
        this.dadmodec = 0;
    this._clearMove();
    ...
    if (!this.dragMove || this.dragMove()) for (var f = !d || !d._clearMove || !d._dragged ? Array(a.parentObject) : d._dragged, e = c.parentObject, g = 0; g < f.length; g++) {
        ...
        this.dadmodec && h !== !1 && (e = this._globalIdStorageFind(h, !0, !0));
...
dhtmlXTreeObject.prototype._dragIn = function(a, b, c, d) {
    ...
    var f = b.parentObject,
        e = a.parentObject;
    if (e || !this._ddronr) {
        if (!this.callEvent("onDragIn", [f.id, e ? e.id: null, f.treeNod, this])) return e && this._autoScroll(a),
            0;
        if (e) {
            if (f.childNodes == null) return this._setMove(a, c, d),
                a;
            for (var g = f.treeNod,
                     h = 0; h < g._dragged.length; h++) if (this._checkPNodes(e, g._dragged[h])) return this._autoScroll(a),
                0;
            this._setMove(a, c, d);
            if (this._getOpenState(e) <= 0) this._autoOpenId = e.id,
                this._autoOpenTimer = window.setTimeout(new callerFunction(this._autoOpenItem, this), 1E3)
...
dhtmlXTreeObject.prototype.moveItem = function(a, b, c, d) {
    ...
    switch (b) {
        ...
        case "item_child":
            ...
            if (!e) return 0; (d || this)._moveNodeTo(f, e, 0);
            break;
        case "item_sibling":
            e = (d || this)._globalIdStorageFind(c);
            if (!e) return 0; (d || this)._moveNodeTo(f, e.parentObject, e);
            break;
        case "item_sibling_next":
            e = (d || this)._globalIdStorageFind(c);
            if (!e) return 0;
            e.tr && e.tr.nextSibling && e.tr.nextSibling.nodem ? (d || this)._moveNodeTo(f, e.parentObject, e.tr.nextSibling.nodem) : (d || this)._moveNodeTo(f, e.parentObject);
            break;
        case "left":
        ...
        case "down_strict":
            var g = this._getIndex(f),
                h = f.parentObject.childsCount - 2;
    ...
    if (_isIE && _isIE < 8) this.allTree.childNodes[0].border = "1",
        this.allTree.childNodes[0].border = "0"
};
...
dhtmlXTreeObject.prototype._getAllChecked = function(a, b, c) {
    ...
    for (var d = a.childsCount,
             f = 0; f < d; f++) b = this._getAllChecked(a.childNodes[f], b, c);
    return b ? b: ""
};
...
dhtmlXTreeObject.prototype._dp_init = function(a) {
    ...
    a._getRowData = function(a) {
        for (var c = {},
                 d = this.obj._globalIdStorageFind(a), f = d.parentObject, e = 0, e = 0; e < f.childsCount; e++) if (f.childNodes[e] == d) break;
        c.tr_id = d.id;
        c.tr_pid = f.id;
        c.tr_order = e;
